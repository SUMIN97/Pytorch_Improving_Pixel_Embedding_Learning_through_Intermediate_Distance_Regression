...
class Trainer2():
    ...
    def train(self, epochs, lamda1, lamda2):
        ...
        for epoch in range(epochs):
            ...
            for idx, data in enumerate(self.train_loader):
                ...
                for i in range(bs):
                    ...
                    inter = self.get_inter(means, neighbor[i], self.E_channels, epoch)

                    # loss_inner, mu, c, d = embedding_loss_single_example(norm_embedding[i], labelmap[i], neighbor[i])
        ...
        torch.save({
            'W_Net': self.wnet.state_dict()
        }, os.path.join(self.save_model_path, 'epoch_{}.pth'.format(epochs)))
    ...
    def get_inter(self, means, neighbor, n_emb, epoch):
        ...
        return inter_mean / bg_include_n_labels
    ...
    def validation(self, epoch):
        for idx, data in enumerate(self.val_loader):
            ...
            for i in range(bs):
                means, intra = self.get_means_intra(labelmap[i], norm_embedding[i])
                inter = self.get_inter(means, neighbor[i], self.E_channels, 0)
