...
save_model_path = '/home/lab/ssd1/Plant/PEIS/models/Model2_Bcnorm2_RealDistanceRegrssion_A1/epoch_487_319.1750.pth'
save_dir = os.path.join('./final_imgs', save_model_path.split('/')[-2])
...
for idx, data in enumerate(test_loader):
    ...
    for i in range(bs):
        name = names[i]
        print(name)
        plant_folder = os.path.join(save_folder, name)
        if os.path.isdir(plant_folder) is False:
            os.mkdir(plant_folder)

        #save rgb
        shutil.copy(src = os.path.join('/home/lab/ssd1/Plant/Improving_Pixel_Embedding_Learning/data/raw/CVPPP2017_LSC_test_data/A1',
                                        name, name+'_rgb.png'),
                    dst=plant_folder)
